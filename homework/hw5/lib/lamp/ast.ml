...
type pty =
  | Mono of ty  (** Monomorphic type, i.e., types with no quantifier *)
  | Scheme of Vars.t * ty  (** type scheme, i.e., universally quantified type *)
...
module Pretty = struct
  ...
  (** Pretty print a [pty] *)
  let pp_pty : pty Fmt.t =
    let open Fmt in
    fun ppf -> function
      | Mono t -> pp_ty ppf t
      | Scheme (xs, t') ->
          if Vars.size xs > 0 then
            pf ppf "@[∀ %a.@ %a@]"
              (hbox (list ~sep:sp string))
              (Vars.to_list xs) pp_ty t'
          else pf ppf "%a" pp_ty t'

  (** Convert a [ty] to a string *)
  let show_ty : ty -> string = Fmt.to_to_string pp_ty

  (** Convert a [pty] to a string *)
  let show_pty : pty -> string = Fmt.to_to_string pp_pty
...
module DSL = struct
  ...
  let lamt ~t x e = Lambda (Some t, Scope (x, e))
  let v x = Var x
  let i n = Num n
  ...
  (** Lift a monomorphic type into a polymorphic type with no quantifier *)
  let mono (t : ty) : pty = Mono t

  (** Universally quantify a monomorphic type *)
  let for_all (xs : string list) (t : ty) : pty = Scheme (Vars.of_list xs, t)
end
